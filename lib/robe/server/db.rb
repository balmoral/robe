...
module Robe
  module Server
    class DB
      ...
      def mongo_client
        if config.use_mongo?
          unless defined?(@mongo_client)
            trace __FILE__, __LINE__, self, __method__, " : creating mongo client for host=#{config.mongo_host} port=#{config.mongo_port} database=#{config.mongo_database}"
            ...
            if @mongo_client
              trace __FILE__, __LINE__, self, __method__, " : created mongo client for host=#{config.mongo_host} port=#{config.mongo_port} database=#{config.mongo_database}"
            else
              raise Robe::DBError, "#{__FILE__}[#{__LINE__}] : could not create mongo client for host=#{config.mongo_host} port=#{config.mongo_port} database='#{config.mongo_database}'"
            end
      ...
      #
      # Is a synchronous operation - no promise.
      #
