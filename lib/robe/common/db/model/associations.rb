...
module Robe; module DB; class Model
  module Associations
    module ClassMethods
      ...
      def embeddings
        @embeddings ||= superclass < Robe::DB::Model && superclass.respond_to?(:embeddings) ? superclass.embeddings.dup : {
          embeds_one: [],
          embeds_many: []
        }
      end
      ...
      def belongs_to_association(collection)
        trace __FILE__, __LINE__, self, __method__, " : collection=#{collection}"
        result = belongs_to_associations.find { |a| a.collection == collection }
