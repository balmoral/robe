...
module Robe; module Client; module Browser; module DOM
  ...
  class Tag
    ...
    # :name,
    %i(id css style name autocomplete enabled disabled selected checked value type autofocus required for data aria on props href src properties height width content).each do |attr|
      if attr == :content
